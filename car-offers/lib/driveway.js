...
async function _getDrivewayOfferImpl({ vin, mileage, zip, condition, email, consumerId, fingerprintProfileId, proxyZip }) {
  ...
  try {
    ...
    // --- Step 1: VIN ---
    checkTimeout();
    await clickFirstByText(page, ['VIN', 'Enter VIN', 'By VIN'], 2000);
    await humanDelay(400, 900);

    // TODO(selector): Driveway VIN input selectors.
    ...
    const vinSubmit = await clickFirstByText(page,
      ['Get Your Offer', 'Get My Offer', 'Get Offer', 'Continue', 'Next'], 3000);
    ...
    for (let step = 0; step < 20; step++) {
      ...
      if (!filledZip) {
        const z = await firstVisible(page, [
      ...
      if (!filledEmail) {
        const e = await firstVisible(page, [
