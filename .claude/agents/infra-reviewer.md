...
14. **Visual-lint review** (per `SKILLS/visual-lint.md`). On any diff that adds or modifies UI (HTML templates, CSS, JS that affects rendering):
    ...
    - (b) If the project has a user-facing surface, `qa.yml` includes the `visual-review-orchestrator.sh` step → else FAIL.
    - (c) For any project with a user-facing surface, `/opt/<project>/PROJECT_CONTEXT.md` must contain a `## User Journeys` section with at least one journey declared (schema: Entry URL, Declared success, Steps, Key failure modes). A missing or TBD-placeholder Journeys section on a user-facing-surface project → FAIL. Infra-only projects use the `_None — infrastructure project with no end-user surface._` placeholder and pass trivially.
    ...
    - (e) For UI-shipping features, the ship must have an associated user journey in `PROJECT_CONTEXT.md#user-journeys` that the change extends, modifies, or introduces. The spec's journey reference (per CLAUDE.md "Clarify Before Building" + the spec template in `SKILLS/user-info-conventions.md`) must match reality. Missing journey → FAIL. Also: CI must have run `acceptance-rehearsal` against that journey and attached the `user_narrative` to `CHANGES.md`. Missing rehearsal output → FAIL. See `SKILLS/acceptance-rehearsal.md`.
