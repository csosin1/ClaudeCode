...
from carmax_abs.config import AUTO_LOAN_NS
...
def _get_credit_score(element, tag: str = "obligorCreditScore") -> Optional[int]:
    """Extract credit score, handling combined-score text format.

    Older ABS-EE XML: plain integer like "714".
    Newer vintages may report two scores in one field, e.g. "VS 714, FICO 774"
    (Vantage + FICO). We prefer FICO when present. Falls back to the first
    integer in the string. Mirrors carvana_abs/ingestion/xml_parser.py.
    """
    text = _get_text(element, tag)
    if not text:
        return None
